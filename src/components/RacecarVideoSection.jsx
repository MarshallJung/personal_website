...
export default function RacecarVideoSection() {
  ...
  const shortGt4 = racecarVideos.find((v) => v.id === 'gt4-chase') || racecarVideos[0];
  const shortRocker = racecarVideos.find((v) => v.id === 'rocker-suspension') || racecarVideos[1];
  const horizPedal = racecarVideos.find((v) => v.id === 'heel-toe-demo') || racecarVideos[2];

  const handleImgError = (id, fallback) => {
    setImgErrors((prev) => ({ ...prev, [id]: fallback }));
  };

  const renderShortCard = (video, isRightCol = false) => (
    <div
      className={`p-6 sm:p-8 flex flex-col justify-between ${
        isRightCol ? 'md:hairline-l' : ''
      }`}
    >
      <div>
        {/* Header Info */}
          ...
          <div className="relative w-full max-w-[260px] sm:max-w-[280px] mx-auto aspect-[9/16] bg-black overflow-hidden hairline-all group shadow-md">
            {activeEmbed === video.id ? (
              ...
              >
                {/* Thumbnail Image */}
                <img
                  src={imgErrors[video.id] || video.thumbnail}
                  alt={video.title}
                  onError={() => handleImgError(video.id, video.fallbackThumbnail)}
        ...
        <div className="text-[11px] font-mono text-[var(--text-muted)] leading-relaxed mb-6">
          <span className="text-[var(--text-main)] font-semibold">Dynamics:</span> {video.techNote}
        </div>
      </div>
        ...
        <div className="p-6 sm:p-8 hairline-b flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <h2 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-[var(--text-main)] uppercase">
              Track Video &amp; Suspension Dynamics
            </h2>
          </div>
          <div className="text-xs font-mono text-[var(--text-muted)] tracking-wider uppercase">
            High Plains Raceway &amp; Road Shakedowns · 3 Video Logs
          </div>
        </div>

        {/* =========================================================================
            TIER 1: Duo Vertical YouTube Shorts (9:16 Aspect Ratio) Side-by-Side
           ========================================================================= */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-0 hairline-b">
          {renderShortCard(shortGt4, false)}
          {renderShortCard(shortRocker, true)}
        </div>

        {/* =========================================================================
            TIER 2: Standard Horizontal Onboard Video (16:9 Aspect Ratio)
           ========================================================================= */}
        <div className="p-6 sm:p-8">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-center">
            {/* 16:9 Video Preview Column */}
            <div className="lg:col-span-7">
              <div className="relative w-full aspect-video bg-black overflow-hidden hairline-all group shadow-md">
                {activeEmbed === horizPedal.id ? (
                  <div className="w-full h-full relative">
                    <iframe
                      src={`https://www.youtube-nocookie.com/embed/${horizPedal.youtubeId}?autoplay=1&rel=0`}
                      title={horizPedal.title}
                      className="w-full h-full border-0"
                      allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
                      allowFullScreen
                    />
                  ...
                  >
                    {/* Thumbnail Image */}
                    <img
                      src={imgErrors[horizPedal.id] || horizPedal.thumbnail}
                      alt={horizPedal.title}
                      onError={() => handleImgError(horizPedal.id, horizPedal.fallbackThumbnail)}
                      className="w-full h-full object-cover object-center group-hover:scale-105 transition-transform duration-500 ease-out"
                      loading="lazy"
                    />
            ...
            <div className="lg:col-span-5 flex flex-col justify-between h-full">
              <div>
                ...
                <h3 className="text-xl sm:text-2xl font-bold tracking-tight text-[var(--text-main)] uppercase mb-2">
                  {horizPedal.title}
                </h3>
                ...
                <div className="p-4 bg-[var(--bg-surface)] hairline-all mb-4">
                  <p className="text-xs sm:text-sm text-[var(--text-main)] italic leading-relaxed">
                    "{horizPedal.description}"
                  </p>
                </div>

                {/* Technical Footnote */}
                <div className="text-[11px] font-mono text-[var(--text-muted)] leading-relaxed mb-6">
                  <span className="text-[var(--text-main)] font-semibold">Kinematics:</span> {horizPedal.techNote}
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-2 pt-4 hairline-t">
                ...
                {activeEmbed !== horizPedal.id ? (
                  <button
                    type="button"
                    onClick={() => setActiveEmbed(horizPedal.id)}
                    className="py-2.5 px-3 text-xs font-mono uppercase tracking-wider text-[var(--text-main)] hairline-all hover:bg-[var(--text-main)] hover:text-[var(--bg-surface)] transition-colors"
                    title="Play video right inside page"
                  >
                    Play In-Page
